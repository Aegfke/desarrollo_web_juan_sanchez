...
const updateComuna = () => {
    ...
    if (data.regiones[selectedRegion]) {
        data.regiones[selectedRegion].comunas.forEach(comuna => {
            let option = document.createElement("option");
...
const updateTiempoInicial = () => {

    let tiempoISelect = document.getElementById("tiempo-inicio");

    tiempoISelect.innerHTML = "";
    ...
    const hours = inicio.getHours().toString().padStart(2, '0');
    const minutes = inicio.getMinutes().toString().padStart(2, '0');

    tiempoISelect.value = `${year}-${month}-${day}T${hours}:${minutes}`;
}

const updateTiempoFinal = () => {
    let tiempoISelect = document.getElementById("tiempo-inicio");
    let tiempoFSelect = document.getElementById("tiempo-final");

    tiempoFSelect.innerHTML = "";

    const inicio = new Date(tiempoISelect.value);

    const final = new Date(inicio);

    final.setHours(inicio.getHours() + 3);

    const year = final.getFullYear();
    const month = (final.getMonth() + 1).toString().padStart(2, '0');
    const day = final.getDate().toString().padStart(2, '0');
    const hours = final.getHours().toString().padStart(2, '0');
    const minutes = final.getMinutes().toString().padStart(2, '0');

    document.getElementById("tiempo-final").value = `${year}-${month}-${day}T${hours}:${minutes}`;
}
...
function agregarInput() {
    let box = document.getElementById("contenedor-files");
    let number = box.querySelectorAll('input[type="file"]').length;
    if (number > 4) {
        return;
    }
...
document.getElementById("tiempo-inicio").addEventListener("change", updateTiempoFinal);
